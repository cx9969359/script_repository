...
def get_record(region, doing_list, ignore_list, objects, crop_threshold, crop_size):
    ...
    mask_arr = mask_arr[us: us + crop_size, ls:ls + crop_size]
    points = mask_arr.tolist()
    result = [x1, y1, x2, y2] + points
    return result
...
def generate_contour_to_draw_image(regions_record, pyvips_image, palette_list, label_list, ignore_label_index,
    ...
    for region in regions_record:
        ...
        if np.array_equal(output_mat, check_mat):
            ...
            text = '{},{},{},{}'.format(region[0], region[1], region[2], region[3])
            draw = ImageDraw.Draw(blend_img)
            font = ImageFont.truetype('C:\Windows\Fonts\simsun.ttc', size=36)
            draw.text((30, 50), text, fill='red', font=font)
...
if __name__ == '__main__':
    args = parse_args()
    image_file_list = scan_files_and_create_folder(args.input_image_path, args.output_image_path, args.ext_list)
    ...
    for image_name in image_file_list:
        ...
        if image_name in xml_file_list:
            ...
            regions_record = get_regions_record(annotation_xml_tree, args.doing_list, args.ignore_list, regions,
                                                args.crop_size, crop_threshold=0.)
            ...
            generate_contour_to_draw_image(regions_record, pyvips_image, palette_list, label_list,
                                           args.ignore_label_index, image_name, args.output_image_path)
