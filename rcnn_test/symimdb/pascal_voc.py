...
class PascalVOC(IMDB):
    ...
    def __init__(self, image_set, root_path, devkit_path):
        ...
        """
        super(PascalVOC, self).__init__(image_set, root_path)
        ...
        self._image_index_file = os.path.join(devkit_path,  'ImageSets', 'Main', image_set + '.txt')
        self._image_file_tmpl = os.path.join(devkit_path,  'JPEGImages', '{}.png')
        self._image_anno_tmpl = os.path.join(devkit_path,  'Annotations', '{}.xml')

        # results
        result_folder = os.path.join(devkit_path, 'results', 'Main')
    ...
    def _load_annotation(self, index):
        ...
        print(self._image_anno_tmpl.format(index))
    ...
    def _do_python_eval(self, all_boxes, use_07_metric):
        ...
        for cls, ap in zip(self.classes[1:], aps):
