...
def get_resnet_feature(data, units, filter_list, bottle_neck=True):
    # res1
    ...
    assert len(units) == 4, "the length of units must be 4"
    unit = pool0

    for i in range(3):
        unit = residual_unit(data=unit, num_filter=filter_list[i], stride=(1 if i==0 else 2, 1 if i == 0 else 2), dim_match=False, name="stage{}_unit{}".format(str(i+1), 1), bottle_neck=bottle_neck)

        for j in range(2, units[i] + 1):
            unit = residual_unit(data=unit, num_filter=filter_list[i], stride=(1, 1), dim_match=True, name='stage{}_unit{}'.format(str(i+1), j), bottle_neck=bottle_neck)

    return unit

def get_resnet_top_feature(data, units, filter_list, bottle_neck=True):
    unit = residual_unit(data=data, num_filter=filter_list[3], stride=(2, 2), dim_match=False, name='stage4_unit1', bottle_neck=bottle_neck)
    for i in range(2, units[3] + 1):
        unit = residual_unit(data=unit, num_filter=filter_list[3], stride=(1, 1), dim_match=True, name='stage4_unit%s' % i, bottle_neck=bottle_neck)
...
def get_resnet_train(anchor_scales, anchor_ratios, rpn_feature_stride,
                     ...
                     units, filter_list, bottle_neck=True):
    ...
    conv_feat = get_resnet_feature(data, units=units, filter_list=filter_list, bottle_neck=bottle_neck)
    ...
    # rcnn top feature
    top_feat = get_resnet_top_feature(roi_pool, units=units, filter_list=filter_list, bottle_neck=bottle_neck)
