...
def load_test_img(im_orig, short, max_size, mean, std):
    ...
    return im_tensor, im_info, im_orig
'''
def load_test_imgs(ims_orig, short, max_size, mean, std):
    im_tensor = ""
    im_info = ""
    ims_orig_list = []
    for im_orig in ims_orig:
      im, im_scale = resize(im_orig, short, max_size)
      height, width = im.shape[:2]
      tmp_im_info = mx.nd.array([height, width, im_scale])
  
      # transform into tensor and normalize
      tmp_im_tensor = transform(im, mean, std)
  
      # for 1-batch inference purpose, cannot use batchify (or nd.stack) to expand dims
      tmp_im_tensor = mx.nd.array(tmp_im_tensor).expand_dims(0)
      tmp_im_info = mx.nd.array(tmp_im_info).expand_dims(0)
      
      if im_tensor == "":
        im_tensor = tmp_im_tensor
        im_info = tmp_im_info
      else:
        im_tensor = mx.nd.stack(im_tensor, tmp_im_tensor)
        im_info = mx.nd.stack(im_info, tmp_im_info)
  
      # transform cv2 BRG image to RGB for matplotlib
      ims_orig_list.append(im_orig[:, :, (2, 1, 0)])
      
    return im_tensor, im_info, ims_orig_list
'''
